...
RSpec.describe Potty::Border do
  ...
  let(:rect) { Potty::Layout::Rect.new(0, 0, 6, 4) }
  ...
  it 'centers a title on the top edge' do
    described_class.draw(window, Potty::Layout::Rect.new(0, 0, 12, 4), title: 'Hi')
    expect(window.strs).to include(' Hi ')
  end

  it 'no-ops on a degenerate rect' do
    described_class.draw(window, Potty::Layout::Rect.new(0, 0, 1, 1))
