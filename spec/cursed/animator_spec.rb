...
RSpec.describe Cursed::Animator do
  let(:app) { CursedSpec.app }
  let(:t0) { Time.at(1_000) }

  # 4-frame looping sprite at 10fps -> 0.1s per frame.
  let(:loop_sprite) do
    Cursed::Sprite.new(:loop, frames: %w[a b c d], fps: 10, mode: :loop)
  end

  let(:once_sprite) do
    Cursed::Sprite.new(:once, frames: %w[x y z], fps: 10, mode: :once)
  end
  ...
  it 'reports preferred_height from the active sprite' do
    tall = Cursed::Sprite.new(:tall, frames: ["1\n2\n3"])
    animator << tall
