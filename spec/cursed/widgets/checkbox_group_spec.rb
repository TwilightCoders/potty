...
RSpec.describe Cursed::Widgets::CheckboxGroup do
  ...
  it 'toggles the option under the cursor on space' do
    group.handle_key(Cursed::Keys::SPACE)        # toggle :a on
    group.handle_key(Cursed::Keys::DOWN)
    group.handle_key(Cursed::Keys::DOWN)
    group.handle_key(Cursed::Keys::SPACE)        # toggle :c on
    expect(group.selected).to eq(%i[a c])
  end

  it 'toggles off when pressed again' do
    group.handle_key(Cursed::Keys::SPACE)        # :a on
    group.handle_key(Cursed::Keys::SPACE)        # :a off
    expect(group.selected).to eq([])
  end

  it 'wraps cursor movement' do
    group.handle_key(Cursed::Keys::UP)           # 0 -> last
    group.handle_key(Cursed::Keys::SPACE)
