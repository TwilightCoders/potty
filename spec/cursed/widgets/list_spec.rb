...
RSpec.describe Cursed::Widgets::List do
  let(:app) { CursedSpec.app }
  subject(:list) { described_class.new(app) }

  def sep(text = '')
    Cursed::Widgets::SeparatorItem.new(text)
  end

  def action(text, &blk)
    Cursed::Widgets::ActionItem.new(text, &blk)
  end
  ...
  describe 'navigation' do
    it 'skips disabled items and emits :select' do
      ...
      list.handle_key(Cursed::Keys::DOWN) # a -> (skip sep) -> b
    ...
    it 'activates the selected item and emits :activate' do
      ...
      list.handle_key(Cursed::Keys::DOWN)  # -> b
      list.handle_key(Cursed::Keys::ENTER) # activate
